...
void dx_plugin::plugin_init() const {
    return;
}

void dx_plugin::plugin_exit() const {
    return;
}

void dx_plugin::plugin_main(const aaediclock_FRect& dims) const {
    ...
    fontcolor.b=128;
    fontcolor.a=0;
    location = host_api->AaediHAM_ConfigGetDX();
    ...
    host_api->AaediHAM_GraphicsDrawText("DX:", fontcolor, TextRect);
    dx_pin.owner=0;
    sprintf(dx_pin.label, "DX");
    ...
    dx_pin.lat=location.lat;
    dx_pin.lon=location.lon;
    dx_pin.icon=0;
    dx_pin.color=fontcolor;
    dx_pin.color.a=255;
    dx_pin.tooltip[0]=0;
...
const char* dx_plugin::getName() const {
    return "DX Module";
}

void dx_plugin::set_host(aaediclock_host_api* host) {
    host_api = host;
}
