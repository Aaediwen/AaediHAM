...
int SDLCALL  fetch_sdo (void *data) {
         ...
         Uint64 data_size = 0;
         char* raw_image = 0 ;
        *(host_api->AaediHAM_LogDebug) << "SOLAR: Fetching SDO image from NASA --  ";
         ...
         if (data_size > 10) {
             ...
             try {
                 ...
                 *(host_api->AaediHAM_LogDebug) << "SOLAR: Loaded image size: " << data_size << " bytes\n";
                 ...
                 *(host_api->AaediHAM_LogDebug) << "SOLAR: Error loading SDO Image  " << e.what() << "\n";
             }
             if (temp) {
...
void sdo_plugin::plugin_main(const aaediclock_FRect& dims) const {
    ...
    *(host_api->AaediHAM_LogDebug) << "SOLAR: In SOLAR module\n";
