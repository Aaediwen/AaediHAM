	...
	if (event->type==SDL_EVENT_MOUSE_BUTTON_UP) {
		if (event->button.button ==1 && event->button.clicks ==1) {
			...
			for (auto& pager : winboxes) {
				if ((event->button.x > pager.panel.dims.x) 
					&& (event->button.x < (pager.panel.dims.x+pager.panel.dims.w)) 
					&& (event->button.y > pager.panel.dims.y) 
					&& (event->button.y < (pager.panel.dims.y+pager.panel.dims.h))) {
					...
					if (pager.plugin_sequence.size()) {
						...
						if (pager.plugin_sequence.size()) {
							...
							debug_log << "EVENT: Panel event owner: " 
								<< clock_mouse_event.plugin_owner 
								<< "("<< loaded_plugins[clock_mouse_event.plugin_owner].name 
								<< ")\n";
						}
					}
				}
