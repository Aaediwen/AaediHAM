...
class map_overlay {
	...
	public:
		map_overlay				();
		~map_overlay				();
		 // return existing if present, or create a new and return
		ScreenFrame* 				get_overlay(SDL_Renderer* renderer, 
								uint16_t owner, SDL_FRect dims, uint8_t z_layer);
		...
		ScreenFrame*				next_overlay(uint8_t z_layer);   // get or use a read-only
											// itterator through overlay_list
		void					reset_index();
