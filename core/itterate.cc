...
//            draw_map(*(master_flags.map.panel));
            ...
            // 14
            aaediclock_FRect module_dims;
            module_dims.w = loaded_plugins[14].host_api->panel->dims.w;
            module_dims.h = loaded_plugins[14].host_api->panel->dims.h;
            module_dims.x = loaded_plugins[14].host_api->panel->dims.x;
            module_dims.y = loaded_plugins[14].host_api->panel->dims.y;
            loaded_plugins[14].plugin->plugin_main(module_dims);
            winboxes[PANEL_MAP].panel.draw_border();
            SDL_SetRenderTarget(clock_renderer, NULL);
