...
double solar_altitude(double lat_deg, double lon_deg, struct tm *utc, double decl_deg) {
	...
	if (!utc || utc->tm_hour < 0 || utc->tm_hour > 23 || utc->tm_min < 0 || utc->tm_min > 59 || utc->tm_sec < 0 || utc->tm_sec > 60) {
		return 0;
	}
...
struct GeoCoord subsolar (const time_t now) {
	...
	//Jean Meeus Astronomical Algorithms Ch 24 (1991)
	...
	// sun's true geometric Longitude and anomaly (MEEUS P152)
	...
	// calculate apparent longitude (MEEUS P152)
	...
	// obliquity of the eleptic per MEEUS 21.2
	// deg + min/60 + sec/3600
	...
	// solar latitude (MEEUS P153)
	// meeus 24.6 + see note after 24.8
